...
The sample Omny feeds also include Podlove Simple Chapters (`psc:chapters`). V1 parses those and writes both remapped Podlove inline chapters and Podcasting 2.0 chapter JSON for completed processed episodes.
...
Conclusion: localized ads are likely coming from podcast hosting/ad infrastructure, not Pocket Casts itself. For the target feeds, Hello Sport is Acast-hosted and The Grade Cricketer/The Circus are Omny/SoundStack/Triton-backed.
...
## Sample Feed Findings

Apple's podcast lookup API returned these feeds:

- Hello Sport: Acast feed
- The Grade Cricketer: Omny feed
- The Circus: Omny feed

Feed inspection showed:

- Hello Sport has Acast metadata and descriptions with sponsor links/signals.
- The Grade Cricketer and The Circus have Omny metadata, audio URLs through SoundStack/Triton-style enrichment/traffic URLs, and some episodes with `psc:chapters`.
- The Circus currently includes useful topic chapters such as NRL, AFL, football, and guest/interview sections.

Conclusion: V1 can already preserve and remap chapters for The Circus and The Grade Cricketer where present.
