...
Conclusion: DeepSeek remains a useful configurable text-only option, but the current default classifier is `qwen/qwen3.6-flash` after the full-episode smoke test where DeepSeek V4 Flash missed the known pre-roll block. The default transcript model is OpenRouter `xiaomi/mimo-v2-omni` in audio-chat mode. OpenRouter responses include `usage.cost` for exact request accounting; the app records that value and uses published token prices only for budget estimates before a request is made.
