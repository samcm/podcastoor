...
Self-hosted open-source ASR has no API cost but has machine time and setup cost. Faster CTranslate2-based runners exist for Whisper-family models, but this app no longer includes a self-hosted ASR provider. It does include local WhisperX forced alignment, which aligns an existing transcript back to the waveform without doing another hosted transcription call.
...
Conclusion: with only `OPENROUTER_API_KEY` available, the best immediate transcription default is OpenRouter `xiaomi/mimo-v2-omni` through audio chat with strict timestamped JSON output plus local WhisperX forced alignment for word-level cut boundaries. Text classification is now routed through an OpenAI-compatible chat endpoint selected by `TEXT_LLM_BASE_URL` and `TEXT_LLM_API_KEY`, so deployment can use OpenCode Go or OpenRouter without code changes. In the bounded audio sweep MiMo tied the best phrase-recall score, was cheaper than `openai/gpt-4o-mini-transcribe`, and returned usable utterance-level timestamps. Those timestamps are still model-estimated before alignment. If another provider key is added, the first high-accuracy hosted alignment target should be Qwen3-ASR/Qwen3-ForcedAligner, Mistral Voxtral Mini Transcribe V2, Deepgram Nova-3, or ElevenLabs Scribe v2. Groq direct remains the cheap word-timestamp option, but it still uses Whisper-family models.
...
Conclusion: DeepSeek remains a useful configurable text-only option, and the current generic text default is `deepseek-v4-pro` through an OpenAI-compatible endpoint. The default transcript model is still OpenRouter `xiaomi/mimo-v2-omni` in audio-chat mode. OpenRouter responses include `usage.cost` for exact request accounting; compatible gateways may return zero or omitted cost for subscription-backed calls, in which case the app still records provider/model/token metadata.

OpenRouter's model API also reports audio-capable multimodal models such as Gemini Flash/Lite and OpenAI GPT audio models. Those can accept audio, but they are not a replacement for forced alignment in this service because the renderer still needs reliable start/end seconds for cuts. The current implementation uses the best tested audio-chat model as the OpenRouter-only default, while still treating provider word timestamps or forced alignment as the next precision upgrade.
