...
Conclusion: with only `OPENROUTER_API_KEY` available, the best immediate default is OpenRouter `xiaomi/mimo-v2-omni` through audio chat with strict timestamped JSON output. In the bounded audio sweep it tied the best phrase-recall score, was cheaper than `openai/gpt-4o-mini-transcribe`, and returned usable utterance-level timestamps. Those timestamps are still model-estimated and should not be confused with word-level forced alignment. If another provider key is added, the first high-accuracy alignment target should be Qwen3-ASR/Qwen3-ForcedAligner, Mistral Voxtral Mini Transcribe V2, Deepgram Nova-3, or ElevenLabs Scribe v2. Groq direct remains the cheap word-timestamp option, but it still uses Whisper-family models.
...
Conclusion: V4 Pro is the current default for chapter/category generation and transcript segment classification because the extra reasoning quality is useful for long, messy transcripts while still staying well below frontier closed-model pricing. DeepSeek V4 Pro is text-only on OpenRouter, so it is used after audio has been converted to timestamped text. The default transcript model is now OpenRouter `xiaomi/mimo-v2-omni` in audio-chat mode. OpenRouter responses include `usage.cost` for exact request accounting; the app records that value and uses published token prices only for budget estimates before a request is made.
