...
export function buildServer(config: AppConfig) {
  ...
  app.get("/", async (_request, reply) => {
    ...
    return renderPodcastList(await listPodcastSummaries(config), getAutomationState());
  });
...
function renderPodcastList(podcasts: Awaited<ReturnType<typeof listPodcastSummaries>>, automation: ReturnType<typeof getAutomationState>): string {
  return page(
    "Podcast Proxy",
    `<header>
      <h1>Podcast Proxy</h1>
...
function renderAudioCompare(episode: UiEpisode): string {
  ...
  return `<section class="compare">
    ${episode.ui.sourceAudioUrl ? `<div><h3>Original Download</h3><audio data-role="source-audio" controls preload="metadata" src="${episode.ui.sourceAudioUrl}"></audio></div>` : ""}
    ${episode.ui.processedAudioUrl ? `<div><h3>Processed</h3><audio data-role="processed-audio" controls preload="metadata" src="${episode.ui.processedAudioUrl}"></audio></div>` : ""}
  </section>`;
}
