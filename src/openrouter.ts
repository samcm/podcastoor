...
export async function classifyTranscriptWithOpenRouter(
  ...
  const prompt = [
    ...
    "Task:",
    "Identify ad, sponsorship, promo, donation, gambling, VPN, merch, ticket, app-install, network privacy, and dead-air/noise segments.",
    "Return segment index ranges plus optional boundary offsets inside the first and last segment.",
    "Offsets are seconds from the segment start. Use them only for mixed transition segments where the ad starts or ends mid-segment.",
...
async function postOpenRouterJson(
  ...
  if (!response.ok) {
    throw new Error(`OpenRouter ${purpose} failed: ${response.status} ${await response.text()}`);
  }
...
function normalizeModelChapters(chapters: Chapter[]): Chapter[] {
  return chapters
    ...
    .filter((chapter) => chapter.title && !/\b(ad|advertisement|sponsor|sponsored|promo|neds|shopify|betterhelp|sprite|sbs insight|good day|yo pro)\b/i.test(chapter.title))
    .map((chapter) => ({
      ...chapter,
      title: chapter.title
        .split(/\s+/)
